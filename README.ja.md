...
フロントマターの `previous` プロパティに、**前のノートへのリンク** を設定するとことで、ノートを連続したシーケンスとして繋ぎ合わせ、その流れに沿ってスムーズにナビゲーションしたり、ネットワークの全体像を可視化したりすることができます。
...
繋がっているノート群をビジュアルなツリー構造として **Obsidian Canvas にエクスポート** できます。
- 現在のノートから派生するすべての「次のノート」のツリー
- Vault 全体のすべての川（シーケンス）
- フィルタリングされた特定の川

これらを簡単にCanvas上で俯瞰できます。また、ノートのつながりにループ構造がある場合は自動的に検出され、起点のノートに `🔄` アイコンが付与されるため、ネットワークの整合性確認にも役立ちます。
...
- **Export all rivers to canvas** (すべての川をCanvasへ書き出し):
  Vault 全体の `previous` プロパティの繋がりを解析し、ネットワーク全体を Canvas ファイルとして出力します。
- **Export filtered rivers to canvas** (フィルタリングした川をCanvasへ書き出し):
  特定の条件でフィルタリングされたノートのネットワークのサブセットを Canvas に出力します。
